...
Download(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		...
		echo -e "${Error} go-shadowsocks2 版目前不支持非 64 位 构架的服务器安装，请更换系统 !" && rm -rf "${FOLDER}" && exit 1
	fi
...
启用详细日志模式就可以在日志中看到链接者信息(包括链接时间、链接代理端口、链接者 IP、链接者访问的目标域名或 IP 这些非敏感类信息)。"
...
Set(){
	check_installed_status
	echo && echo -e "你要做什么？
 ${Green_font_prefix}1.${Font_color_suffix}  修改端口配置
 ${Green_font_prefix}2.${Font_color_suffix}  修改密码配置
 ${Green_font_prefix}3.${Font_color_suffix}  修改加密配置
 ${Green_font_prefix}4.${Font_color_suffix}  修改详细日志模式配置
 ${Green_font_prefix}5.${Font_color_suffix}  修改全部配置
————————————————
 ${Green_font_prefix}6.${Font_color_suffix}  监控运行状态" && echo
...
ss_link_qr(){
	if [[ "${ipv4}" != "IPv4_Error" ]]; then
		...
		SSurl="ss://${SSbase64}"
		SSQRcode="http://localhost/qr/qr.php?text=${SSurl}"
...
View_Log(){
	...
	echo -e "如果想要查看详细日志，请在 [7.设置 账号配置 - 4.修改详细日志模式配置] 开启。" && echo
	tail -f ${LOG}
}
# 显示 Shadowsocks 连接信息
...
crontab_monitor_cron_start(){
	...
	if [[ -z ${cron_config} ]]; then
		echo -e "${Error} Shadowsocks 服务端运行状态监控功能启动失败!" && exit 1
	else
		echo -e "${Info} Shadowsocks 服务端运行状态监控功能启动成功!"
	fi
}
crontab_monitor_cron_stop(){
	crontab -l > "$file_1/crontab.bak"
	...
	cron_config=$(crontab -l | grep "ss-go.sh monitor")
	if [[ ! -z ${cron_config} ]]; then
		echo -e "${Error} Shadowsocks 服务端运行状态监控功能停止失败!" && exit 1
	else
		echo -e "${Info} Shadowsocks 服务端运行状态监控功能停止成功!"
	fi
}
crontab_monitor(){
	check_installed_status
	...
	#echo "${PID}"
	if [[ -z ${PID} ]]; then
		echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] 检测到 Shadowsocks 服务端未运行 , 开始启动..." | tee -a ${LOG}
		/etc/init.d/ss-go start
		sleep 1s
		check_pid
		if [[ -z ${PID} ]]; then
			echo -e "${Error} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] Shadowsocks服务端启动失败..." | tee -a ${LOG}
		else
			echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] Shadowsocks服务端启动成功..." | tee -a ${LOG}
		fi
	else
		echo -e "${Info} [$(date "+%Y-%m-%d %H:%M:%S %u %Z")] Shadowsocks服务端进程运行正常..." | tee -a ${LOG}
	fi
}
Add_iptables(){
	default_zone=$(firewall-cmd --get-default-zone)
  firewall-cmd --permanent --zone=${default_zone} --add-port=${ss_port}/tcp
  firewall-cmd --permanent --zone=${default_zone} --add-port=${ss_port}/udp
  firewall-cmd --reload
}
Del_iptables(){
	default_zone=$(firewall-cmd --get-default-zone)
  firewall-cmd --permanent --zone=${default_zone} --remove-port=${ss_port}/tcp
  firewall-cmd --permanent --zone=${default_zone} --remove-port=${ss_port}/udp
  firewall-cmd --reload
}
#Save_iptables(){
#	if [[ ${release} == "centos" ]]; then
...
————————————
 ${Green_font_prefix} 7.${Font_color_suffix} 设置账号配置
 ${Green_font_prefix} 8.${Font_color_suffix} 查看账号信息
 ${Green_font_prefix} 9.${Font_color_suffix} 查看日志信息
 ${Green_font_prefix}10.${Font_color_suffix} 查看链接信息
————————————" && echo
	if [[ -e ${FILE} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
